...
function saveCartValues(){
    ...
    cart.length > 0 && cart.map((item) => (itemsTotal +=item.price.newPrice * item.quantity)); //map: dizinin her elemanı için bu işlemi yap 
    
    subTotal.innerHTML = `$${itemsTotal.toFixed(2)}`;
    cartTotal.innerHTML = `$${itemsTotal.toFixed(2)}`;
    

    fastCargo.addEventListener("change", function(e){  //change: check olup olmadığını kontrol için
      if(e.target.checked){
